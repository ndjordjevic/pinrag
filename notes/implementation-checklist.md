...
- [x] **Web documentation indexing** — Index documentation sites from any http(s) URL. Discovery chain: `llms.txt` / `llms-full.txt` (Mintlify-style markdown index) → `sitemap.xml` (+ `robots.txt` `Sitemap:` hints, recursive sitemap indexes) → scoped BFS crawl. Scope boundary: exact host + path prefix derived from the seed URL (no subdomains). Fetch via `httpx` async with per-host token-bucket rate limiting, `urllib.robotparser` for robots.txt, and size caps. Extraction: `text/markdown` pass-through; HTML through `trafilatura` with a BeautifulSoup + `markdownify` fallback scoped to `<main>` / `<article>` / `[role=main]`. Metadata: `document_id` (`<host><path_prefix>`), `document_type` ("web"), `source_url` per chunk, `page_title`. Parent-child chunks supported. Env: `PINRAG_WEB_*` (max pages, depth, concurrency, rate, user agent, respect robots, prefer llms.txt). Known limitation: Cloudflare-protected sites may return 403.
